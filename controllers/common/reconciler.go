...
func (r *Reconciler) prepare(req ctrl.Request, instance v1.Instance) (*Transaction, error) {
	...
	secretsClient := r.secrets(ctx, instance, &logger)
...
func (r *Reconciler) process(tx *Transaction) error {
	...
	tx.Logger = tx.Logger.WithField("ClientID", instanceClient.ClientID)
...
func (r *Reconciler) createClient(tx *Transaction, payload types.ClientRegistration) (*types.ClientRegistration, error) {
	...
	tx.Logger.WithField("ClientID", registrationResponse.ClientID).Info("client registered")
...
func (r *Reconciler) registerJwk(tx *Transaction, jwk jose.JSONWebKey, clientID string) error {
	jwks, err := crypto.MergeJwks(jwk, tx.ManagedSecrets.Used, tx.Instance.GetSecretMapKey())
	if err != nil {
