package ansattportenclient_test
...
func TestMain(m *testing.M) {
	handler := test.AnsattportenHandler(test.ClientID)
...
func TestAnsattportenController(t *testing.T) {
	cfg := fixtures.Config{
	...
	clusterFixtures := fixtures.New(cli, cfg).MinimalConfig(clients.AnsattportenTypeLabelValue).WithNamespace()

	// create AnsattportenClient
	if err := clusterFixtures.Setup(); err != nil {
		t.Fatalf("failed to set up cluster fixtures: %v", err)
	}

	instance := &nais_io_v1.AnsattportenClient{}
	key := client.ObjectKey{
		Name:      "test-client",
		Namespace: "test-namespace",
	}
	assert.Eventually(t, test.ResourceExists(cli, key, instance), test.Timeout, test.Interval, "AnsattportenClient should exist")
	assert.Eventually(t, func() bool {
		err := cli.Get(context.Background(), key, instance)
		assert.NoError(t, err)
		return clients.IsUpToDate(instance)
	}, test.Timeout, test.Interval, "AnsattportenClient should be synchronized")
	...
	// delete AnsattportenClient
	err = cli.Delete(context.Background(), instance)

	assert.NoError(t, err, "deleting AnsattportenClient")

	assert.Eventually(t, test.ResourceDoesNotExist(cli, key, instance), test.Timeout, test.Interval, "AnsattportenClient should not exist")
}
...
func secretAssertions(t *testing.T) func(*corev1.Secret, clients.Instance) {
	return func(actual *corev1.Secret, instance clients.Instance) {
		...
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenClientIDKey])
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenJwkKey])
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenRedirectURIKey])
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenWellKnownURLKey])
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenIssuerKey])
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenJwksUriKey])
		assert.NotEmpty(t, actual.Data[secrets.AnsattportenTokenEndpointKey])
